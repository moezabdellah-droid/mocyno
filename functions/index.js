...
// === CONFIG ===
// Utiliser une image hébergée sur votre domaine (meilleure délivrabilité d’images en email).
const LOGO_URL = "https://mocyno.com/mocyno-logo.png";
const FROM_EMAIL = "no-reply@mocyno.com";   // expéditeur (domaine vérifié dans Resend)
...
app.post("/api/contact", async (req, res) => {
  try {
    ...
    const frameOpen =
      `<div style="font-family:Arial,Helvetica,sans-serif;line-height:1.5;color:#222;background:#f5f7fa;padding:20px">` +
      `<div style="max-width:600px;margin:auto;background:#fff;padding:24px;border-radius:12px;box-shadow:0 3px 10px rgba(0,0,0,.08)">`;

    const header = `
      <div style="text-align:center;margin-bottom:16px">
        <img src="${LOGO_URL}" alt="MO'Cyno" width="120" style="height:auto;margin-bottom:8px;border-radius:8px;display:inline-block">
    ...
    const frameClose =
      ...
      `<p style="font-size:12px;color:#888;text-align:center">Mail envoyé depuis ` +
      `<a href="https://mocyno.com" style="color:#0f62fe;text-decoration:none">mocyno.com</a></p></div></div>`;
...
await Promise.all([
  ...
  resend.emails.send({
    from: FROM_EMAIL,
    to: [ADMIN_EMAIL, "abdellahmoez@gmail.com"], // <= ajoute ton Gmail ici
